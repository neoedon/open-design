...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "viaim Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "viaim Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "viaim Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "viaim Design", "viaim Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "viaim Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "viaim Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "viaim Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "viaim Design", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "viaim Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "viaim Design.exe"))).resolves.toMatchObject({
