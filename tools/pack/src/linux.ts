...
const PRODUCT_NAME = "viaim Design";
const APP_IMAGE_PRODUCT_NAME = "viaim-Design";
...
async function writeLinuxBuilderConfig(config: ToolPackConfig, paths: LinuxPaths): Promise<void> {
  ...
  const builderConfig: Record<string, unknown> = {
    ...
    linux: {
      ...
      synopsis: "viaim Design",
      maintainer: "viaim Design Contributors",
    },
...
export async function installPackedLinuxHeadless(config: ToolPackConfig): Promise<LinuxHeadlessInstallResult> {
  ...
  const script = [
    "#!/bin/sh",
    `# viaim Design headless launcher — namespace: ${config.namespace}`,
